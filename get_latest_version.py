...
PKG_NAME = 'jsonschema-inference'
IMPORT_NAME = 'jsonschema_inference'
...
def write_version_to_file(version_number: str) -> None:
    ...
    version_file_path: str = str(pathlib.Path(
        __file__).parent.absolute()) + f"/{IMPORT_NAME}/version.py"
...
def unpack_version_number(version_string: str) -> Tuple[int, int, int]:
    ...
    return int(version_buffer[0]), int(
        version_buffer[1]), int(version_buffer[2])


def pack_version_number(
        version_buffer: Union[Tuple[int, int, int], List[int]]) -> str:
    """
...
def increase_version_number(
        version_buffer: Union[Tuple[int, int, int], List[int]]) -> List[int]:
    """
